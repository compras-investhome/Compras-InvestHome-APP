// Encargado Module - Lógica del perfil de encargado
...
function showView(viewName) {
    ...
    if (cartButton) {
        if (viewName === 'encargado-tienda' || viewName === 'productos' || viewName === 'categorias') {
    ...
    if (viewName === 'encargado-tienda') {
        loadTiendas();
        updateCartCountAdmin();
    } else if (viewName === 'encargado-pedidos-curso') {
        loadPedidosEnCursoEncargado();
    } else if (viewName === 'encargado-historico') {
        loadHistoricoEncargado();
    }
}
...
async function loadCategorias(tiendaId) {
    ...
    const mainContent = document.querySelector('#view-encargado-tienda .main-content');
...
async function loadMisPedidos() {
    ...
    for (const obra of obras) {
        ...
        for (const pedido of pedidosObra) {
            const card = await createPedidoEncargadoCard(pedido);
...
function switchTabPedidos(tab) {
    // Para encargado solo hay pedidos normales
...
// ========== PEDIDOS ESPECIALES ==========
// Los encargados no pueden crear ni ver pedidos especiales
...
async function openModalPedidoEspecialAdmin(pedidoId = null) {
    if (!currentUser || (currentUserType !== 'Encargado' && currentUserType !== 'Administrador')) {
        showAlert('Solo los encargados y administradores pueden crear/editar pedidos especiales');
        return;
    }
...
async function guardarPedidoEspecial() {
    ...
    if (notas && notas.trim()) {
        pedidoEspecial.notas.push({
            ...
            usuarioTipo: currentUserType || currentUser?.tipo || 'Encargado',
            mensaje: notas,
...
async function loadPedidosEnCursoEncargado() {
    ...
    try {
        ...
        for (const obra of obrasConPedidos) {
            ...
            const card = createObraCardPedidosCursoEncargado(obra, pedidosCount);
...
function createObraCardPedidosCursoEncargado(obra, pedidosCount) {
    const card = document.createElement('div');
    ...
    card.addEventListener('click', () => {
        loadPedidosCursoPorObraEncargado(obra.id || 'sin-obra', obra.nombreComercial || obra.nombre || 'Sin obra asignada');
    });
...
async function loadPedidosCursoPorObraEncargado(obraId, obraNombre) {
    const obrasView = document.getElementById('pedidos-curso-obras-view');
    const pedidosView = document.getElementById('pedidos-curso-pedidos-view');
    ...
    try {
        ...
        cargarMasPedidosCursoEncargado();
    } catch (error) {
...
async function cargarMasPedidosCursoEncargado() {
    const pedidosList = document.getElementById('pedidos-curso-pedidos-list');
    const cargarMasWrapper = document.getElementById('pedidos-curso-cargar-mas-wrapper');
...
function aplicarFiltrosPedidosCursoEncargado(pedidos) {
    let filtrados = [...pedidos];
...
async function populatePedidosCursoFiltersEncargado(pedidosEnCurso) {
...
function setupPedidosCursoFiltersListenersEncargado() {
    const selectObra = document.getElementById('filter-obra-curso');
    const selectTienda = document.getElementById('filter-tienda-curso');
    const selectEstadoEnvio = document.getElementById('filter-estado-envio-curso');
    const selectEstadoPago = document.getElementById('filter-estado-pago-curso');
...
async function loadHistoricoEncargado() {
    ...
    try {
        ...
        for (const obra of obrasConPedidos) {
            const pedidosCount = pedidosPorObra.get(obra.id || 'sin-obra').length;
            const card = createObraCardHistoricoEncargado(obra, pedidosCount);
            obrasList.appendChild(card);
        }
    } catch (error) {
...
function createObraCardHistoricoEncargado(obra, pedidosCount) {
    const card = document.createElement('div');
    card.className = 'obra-card obra-card-cerrados';
    card.style.cursor = 'pointer';
    ...
    // Agregar event listener para mostrar pedidos de esta obra
    card.addEventListener('click', () => {
        loadPedidosHistoricosPorObraEncargado(obra.id || 'sin-obra', obra.nombreComercial || obra.nombre || 'Sin obra asignada');
    });
    
    return card;
}

async function loadPedidosHistoricosPorObraEncargado(obraId, obraNombre) {
    const obrasView = document.getElementById('historico-obras-view');
    const pedidosView = document.getElementById('historico-pedidos-view');
...
async function cargarMasPedidosHistoricosEncargado() {
    const pedidosList = document.getElementById('historico-pedidos-list');
    const cargarMasWrapper = document.getElementById('historico-cargar-mas-wrapper');
    ...
    // Crear cards para los pedidos a cargar
    for (const pedido of pedidosACargar) {
        const card = await createPedidoTiendaCardEncargado(pedido);
        pedidosList.appendChild(card);
    }
...
async function createPedidoTiendaCardEncargado(pedido) {
    const card = document.createElement('div');
    card.className = 'pedido-gestion-card contab-pedido-card';
...
function setupEncargadoEventListeners() {
    try {
    ...
    // Logout
    document.getElementById('btn-logout-encargado')?.addEventListener('click', async () => {
...
async function initEncargado() {
    try {
        console.log('Iniciando encargado...');
        ...
        console.log('Comparación con "Encargado":', usuario?.tipo === 'Encargado');
        console.log('Comparación con "Encargado" (trim):', usuario?.tipo?.trim() === 'Encargado');
        ...
        if (usuario.tipo !== 'Encargado') {
            console.log('Usuario no es encargado. Tipo recibido:', usuario.tipo, 'Tipo esperado: Encargado');
            window.location.href = '../index.html';
            return;
        }
        ...
        console.error('Error al inicializar encargado:', error);
