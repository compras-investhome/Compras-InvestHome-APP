...
async function createPedidoContabilidadCard(pedido, isPagado = false) {
    ...
    const documentoPagoContent = `
        ${tienePago ? `<button type="button" onclick="abrirDocumento(this)" data-document-url="${pedido.transferenciaPDF.replace(/"/g, '&quot;')}" class="doc-link" style="background: none; border: none; cursor: pointer; padding: 0; color: inherit; text-decoration: underline;">📄 Ver pago</button>` : '<span class="doc-placeholder">Sin documento</span>'}
        ${puedeGestionarPago ? (
            tienePago
...
async function createPedidoEspecialContabilidadCard(pedido) {
    ...
    const documentoPagoContent = tieneDocumentoPago
        ? `<button type="button" onclick="abrirDocumento(this)" data-document-url="${pedido.documentoPago.replace(/"/g, '&quot;')}" class="doc-link" style="background: none; border: none; cursor: pointer; padding: 0; color: inherit; text-decoration: underline;">📄 Ver documento de pago</button>`
        : '<span class="doc-placeholder">Sin documento</span>';
